...
class IdentificationManagement(gym.Env):
    def __init__(self, environment_configuration=None):
        ...
        self.bool_to_int = {False: -1, True: 1}
    ...
    def step(self, action: dict):
        ...
        action_is_real_source: int = action['is_real_source']
        ...
        if action_is_real_source != 0:
            ...
            reward = 0
            self._calling_identification_factors(action_calling_identification_factors)
    ...
    def _get_observation(self):
        observation = {
            ...
            'position_current_message': self.position_current_message,
            'current_message_criticality': self.current_message.criticality,
            'current_message_trust': self.current_message.trust,
        }
        return observation
    ...
    def _calling_identification_factors(self, calling: np.ndarray):
        for i in range(self.number_identification_factors):
            if calling[i] == 1 and self.response_identification_factors[i] == 0:
                ...
                if self.current_energy >= factor_energy_cost:
                    ...
                    warnings.warn(f'The agent is attempting to call identification factor {i} for which it does not '
                                  f'have enough energy.')

            elif calling[i] == 1 and self.response_identification_factors[i] != 0:
                warnings.warn(f'The agent is attempting to call the identification factor {i} that has already been '
